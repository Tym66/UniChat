...
class XposedInit : IXposedHookLoadPackage {
    ...
    override fun handleLoadPackage(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        if (lpparam.processName != pkg) return // 只处理主进程
        if (!isModuleActive()) return

        XposedBridge.log("[UniChat] hook 已加载: $pkg (${lpparam.processName})")
        try {
            SqliteHook.install(platform)
        } catch (t: Throwable) {
            XposedBridge.log("[UniChat] SQLite hook 失败: ${t.message}")
        }
    }

    private fun isModuleActive(): Boolean {
        return try {
            XposedHelpers.findClass("de.robv.android.xposed.XposedBridge", null) != null
        } catch (t: Throwable) {
            false
        }
    }
...
object SqliteHook {
    ...
    fun install(platform: String) {
        if (!installed.add(platform)) return
    ...
    private fun reportMessage(platform: String, values: android.content.ContentValues) {
        ...
        if (peerId.isNullOrBlank() || msgId.isNullOrBlank()) return
        // 跳过空内容系统消息
